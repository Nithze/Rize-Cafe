    ...
    <div class="container">
        ...
        <div id="addUserModal" class="modal">
            <div class="modal-content">
                ...
                <h2>Add User</h2>
                <form class="add-form" action="{{ route('addUser') }}" method="POST" id="addUserForm">
                    @csrf
                    ...
                    <br><br> -->
                    <label for="name">Name:</label><br>
                    <input type="text" id="name" name="name" required minlength="4" value="{{ old('name') }}">
                    @if ($errors->has('name'))
                        <div class="error">{{ $errors->first('name') }}</div>
                    @endif
                    <br><br>
                    ...
                    <select class="add-role" id="usertype" name="usertype" required>
                    ...
                    <div class="button-adduser">
                        ...
                        <!-- <button class="add-close" type="button" onclick="closeAddUserForm()">Cancel</button> -->
                    </div>
                </form>
            </div>
            <script>
                document.addEventListener("DOMContentLoaded", function () {
                    @if ($errors->any())
                        ...
                        @foreach ($errors->all() as $error)
                            errorMessages["{{ $loop->index }}"] = "{{ $error }}";
                        @endforeach

                        for (let key in errorMessages) {
                            let errorDiv = document.createElement("div");
                            errorDiv.classList.add("error");
                            errorDiv.innerText = errorMessages[key];
            ...
            <script>
                ...
                document.addEventListener("DOMContentLoaded", function () {
                    // Jika ada error, tetap tampilkan modal
                    @if ($errors->any())
        ...
        <div id="editUserModal" class="modal">
            <div class="modal-content">
                <span class="close" onclick="closeEditUserForm()">&times;</span>
                <h2>Edit User</h2>
                <form class="edit-form" action="{{ route('editUser') }}" method="POST" id="editUserForm"
                    onsubmit="return confirmEditUser()">
                    @csrf
                    @method('PUT')
                    <input type="hidden" id="editUserId" name="id">

                    <label for="editName">Name:</label><br>
                    <input type="text" id="editName" name="name" required minlength="4">
                    @if ($errors->has('name'))
                        <div class="error">{{ $errors->first('name') }}</div>
                    @endif
                    <br><br>
                    ...
                    @if ($errors->has('password'))
                        <div class="error">{{ $errors->first('password') }}</div>
                    @endif
                    <!-- <label for="editPassword">jika kosong pasword tidak di edit</label><br> -->
                    <br><br>

                    <label for="editUsertype">Role:</label><br>
                    <select id="editUsertype" name="usertype" required class="edit-role">
            ...
            <script>
                ...
                function openEditUserForm(userId, userName, userEmail, userType) {
                ...
                document.addEventListener("DOMContentLoaded", function () {
                    document.querySelectorAll('.button-edit').forEach(button => {
                        button.addEventListener('click', function () {
                            ...
                            const userId = userRow.querySelector('td:nth-child(1)').textContent;
                            ...
                            const userType = userRow.querySelector('td:nth-child(4)').textContent;

                            openEditUserForm(userId, userName, userEmail, userType);
                        });
                    });
                });

                function confirmEditUser() {
                    return confirm("Apakah Anda yakin dengan proses ini?");
                }
            </script>
        </div>
                                ...
                                document.addEventListener("DOMContentLoaded", function () {
                                    ...
                                    roleFilter.addEventListener('change', function () {
                                        ...
                                        for (let i = 1; i < rows.length; i++) {
                                            ...
                                            if (selectedRole === 'all' || (role.toLowerCase() === selectedRole.toLowerCase() && (role.toLowerCase() !== 'admin' || selectedRole.toLowerCase() === 'admin'))) {
                                                rows[i].style.display = "";
                                            } else {
                                                rows[i].style.display = "none";
                                            }
                                ...
                                <td>
                                    <div class="action-form">
                                        ...
                                        <form action="{{ route('deleteUser', $user->id) }}" method="POST"
                                            ...
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="button-delete">Hapus</button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
